...
class TestArgsLenLogger(unittest.TestCase):
    ...
    def test_repeat_lens(self):
        for i in range(2):
            ...
            if i == 0:
                ...
                log = self.log_adapt
            @ArgsLen(3, 3, 3, 3, 3, opt_logger=log)
            def foo(*args):
                return len(args)
            with self.assertRaises(ValueError):
                result = foo()
    ...
    def test_ranges(self):
        for i in range(2):
            ...
            @ArgsLen((3, 5), (7, 9), opt_logger=log)
            def foo(*args):
                return len(args)
            with self.assertRaises(ValueError):
                foo()
